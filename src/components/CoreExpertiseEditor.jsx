...
const CoreExpertiseEditor = ({ skills, onSave, onClose }) => {
  ...
  const handleImageUpload = async (id, file, type = 'background') => {
    if (!file) return;

    const formData = new FormData();
    formData.append('image', file);
    formData.append('skill_id', id);
    formData.append('image_type', type); // 'icon' or 'background'
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        alert('Core Expertise updated successfully! Page will reload to show changes.');
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        type="file"
                        accept="image/*"
                        onChange={(e) => handleImageUpload(skill.id, e.target.files[0])}
                        className="hidden"
                        id={`image-${skill.id}`}
                      />
                      <label
                        htmlFor={`image-${skill.id}`}
...
export default CoreExpertiseEditor;
